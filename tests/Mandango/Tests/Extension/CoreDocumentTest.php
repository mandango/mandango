...
class CoreDocumentTest extends TestCase
{
    ...
    {
        $book = $this->mandango->createDocument('Model\Book');
    ...
    {
        $this->mandango->createDocument('Model\Article')->setAuthor($this->mandango->createDocument('Model\Category'));
    }

    public function testReferencesOneGetterQuery()
    {
        $author = $this->mandango->createDocument('Model\Author')->setName('foo')->save();

        $article = $this->mandango->createDocument('Model\Article')->setAuthorId($author->getId());
        $this->assertSame($author, $article->getAuthor());
    }
    ...
    {
        $article = $this->mandango->createDocument('Model\Article')->setAuthorId(new \MongoId('123'));
        $article->getAuthor();
    }
    ...
    {
        $article = $this->mandango->createDocument('Model\Article');
        $category = $this->mandango->createDocument('Model\Category');
    ...
    {
        $author = $this->mandango->createDocument('Model\Author');
        $article = $this->mandango->createDocument('Model\Article')->setAuthor($author);
    ...
    public function testUpdateReferenceFieldsReferencesManyNew()
    {
        ...
        $ids = array();
        for ($i = 1; $i <= 5; $i ++) {
            $categories->add($this->mandango->createDocument('Model\Category')->setId($ids[] = new \MongoId($i)));
        }
        $article->updateReferenceFields();
    ...
    public function testUpdateReferenceFieldsReferencesManyNotNew()
    {
        ...
        $addIds = array();
        for ($i = 1; $i <= 3; $i++) {
            $categories->add($this->mandango->createDocument('Model\Category')->setId($addIds[] = new \MongoId('10'.$i)));
        }
        $categories->remove($this->mandango->createDocument('Model\Category')->setId($baseIds[1]));
        $categories->remove($this->mandango->createDocument('Model\Category')->setId($baseIds[3]));

        $categories = $article->getSource()->getCategories();
        $sourceAddIds = array();
        for ($i = 1; $i <= 2; $i++) {
            $categories->add($this->mandango->createDocument('Model\Category')->setId($sourceAddIds[] = new \MongoId('101'.$i)));
        }
        $categories->remove($this->mandango->createDocument('Model\Category')->setId($sourceBaseIds[1]));

        $comments = $article->getComments()->getSaved();
        $categories = $comments[0]->getCategories();
        $commentAddIds = array();
        for ($i = 1; $i <= 3; $i++) {
            $categories->add($this->mandango->createDocument('Model\Category')->setId($commentAddIds[] = new \MongoId('102'.$i)));
        }
        $categories->remove($this->mandango->createDocument('Model\Category')->setId($commentBaseIds[1]));
    ...
    {
        $article = $this->mandango->createDocument('Model\Article');
        $author1 = $this->mandango->createDocument('Model\Author')->setName('foo');
        $article->setAuthor($author1);
        $source = $this->mandango->createDocument('Model\Source');
        $author2 = $this->mandango->createDocument('Model\Author')->setName('bar');
        $source->setAuthor($author2);
        $article->setSource($source);
        $simpleEmbedded = $this->mandango->createDocument('Model\SimpleEmbedded')->setName('foo');
    ...
    public function testSaveReferencesReferencesMany()
    {
        $articleCategories = array(
            $this->mandango->createDocument('Model\Category')->setName('c1')->save()->setName('c1u'),
            $this->mandango->createDocument('Model\Category')->setName('c2')->save()->setName('c1u'),
            $this->mandango->createDocument('Model\Category')->setName('c3'),
            $this->mandango->createDocument('Model\Category')->setName('c4'),
        );
    ...
    public function testEmbeddedsOneSettersGetters()
    {
        ...
        $source2 = $this->mandango->createDocument('Model\Source');
    ...
    public function testEmbeddedsOneSettersGettersDeep1()
    {
        ...
        $source = $this->mandango->createDocument('Model\Source');
        $article->setSource($source);
        $info = $this->mandango->createDocument('Model\Info');
        $source->setInfo($info);
    ...
    {
        $information = $this->mandango->createDocument('Model\ArticleInformation')->setName('foo')->save();
        $article = $this->mandango->createDocument('Model\Article')->setInformation($information)->save();
    ...
    public function testRelationsManyThrough()
    {
        $users = array();
        for ($i = 1; $i <= 10; $i++) {
            $users[$i] = $user = $this->mandango->createDocument('Model\User');
        ...
        $articlesVotes = array();
        for ($i = 1; $i <= 10; $i++) {
            $articles[$i] = $article = $this->mandango->createDocument('Model\Article');
            $article->setTitle('article'.$i);
            $this->mandango->persist($article);

            for ($z = $i; $z <= 10; $z++) {
                $articleVote = $this->mandango->createDocument('Model\ArticleVote');
    ...
    public function testResetGroups()
    {
        $article = $this->mandango->createDocument('Model\Article')
            // referencesMany
            ->addCategories($this->mandango->createDocument('Model\Category'))
            // embeddedsMany
            ->addComments($this->mandango->createDocument('Model\Comment'))
            // embeddedsOne with groups
            ->setSource($this->mandango->createDocument('Model\Source')
                ->addCategories($this->mandango->createDocument('Model\Category'))
            )
            // embeddedsMany with groups
            ->addComments($comment = $this->mandango->createDocument('Model\Comment')
                ->addInfos($this->mandango->createDocument('Model\Info'))
            )
        ;
        $article->resetGroups();
    ...
    {
        $this->mandango->createDocument('Model\Article')->set('no', 'foo');
    }
    ...
    {
        $this->mandango->createDocument('Model\Article')->get('no');
    }
    ...
    public function testToArrayInitializeFields()
    {
        ...
        $article = $this->mandango->createDocument('Model\Article')->setId($article->getId());
    ...
    public function testReferencesManyQuery()
    {
        ...
        $ids = array();
        for ($i = 1; $i <= 10; $i++) {
            $category = $this->mandango->createDocument('Model\Category')->setName('Category'.$i)->save();
            if ($i % 2) {
        ...
        $article = $this->mandango->createDocument('Model\Article')->setCategoryIds($ids);
    ...
    {
        $this->assertSame(array(), $this->mandango->createDocument('Model\Article')->getComments()->getSaved());
    }
    ...
    public function testRefresh()
    {
        ...
        $article = $this->mandango->createDocument('Model\Article')->setId($articleRaw['_id'])->setTitle('ups')->setNote('bump');
        $article->refresh();
    ...
    public function testRootAndPath()
    {
        ...
        $article2 = $this->mandango->createDocument('Model\Article');
    ...
    public function testSetDocumentDataEmbeddedsOne()
    {
        ...
        $source = $article->getSource();
        $this->assertEquals($this->mandango->createDocument('Model\Source')->setDocumentData($sourceData), $source);
        $this->assertSame(array('root' => $article, 'path' => 'source'), $source->getRootAndPath());
        $info = $source->getInfo();
        $this->assertEquals($this->mandango->createDocument('Model\Info')->setDocumentData($infoData), $info);
        $this->assertSame(array('root' => $article, 'path' => 'source.info'), $info->getRootAndPath());
    }
    ...
    public function testSetDocumentDataEmbeddedsMany()
    {
        ...
        $this->assertEquals($this->mandango->createDocument('Model\Comment')->setDocumentData($commentsData[0]), $savedComments[0]);
        $this->assertEquals($this->mandango->createDocument('Model\Comment')->setDocumentData($commentsData[1]), $savedComments[1]);
        ...
        $this->assertSame(2, count($savedComments));
        $this->assertEquals($this->mandango->createDocument('Model\Info')->setDocumentData($infosData[0]), $savedInfos[0]);
        $this->assertEquals($this->mandango->createDocument('Model\Info')->setDocumentData($infosData[1]), $savedInfos[1]);
    }
    ...
    public function testIsModifiedNewEmbeddedsOne()
    {
        ...
        $info = $this->mandango->createDocument('Model\Info');
        $source = $this->mandango->createDocument('Model\Source')->setInfo($info);
    ...
    public function testIsModifiedNotNewEmbeddedsOne()
    {
        ...
        $this->assertFalse($source->isModified());
        $this->assertFalse($article->isModified());
        $source->setInfo($this->mandango->createDocument('Model\Info'));
        $this->assertTrue($source->isModified());
        $this->assertTrue($article->isModified());
    }
    ...
    public function testIsModifiedNewEmbeddedsMany()
    {
        ...
        $comments = $article->getComments();
        $this->assertFalse($article->isModified());
        $comments->remove($this->mandango->createDocument('Model\Comment')->setName('foo'));
        $this->assertFalse($article->isModified());
        $comment = $this->mandango->createDocument('Model\Comment');
        $comments->add($comment);
        $this->assertFalse($article->isModified());
        ...
        $this->assertFalse($article->isModified());
        $infos->remove($this->mandango->createDocument('Model\Info')->setName('foo'));
        $this->assertFalse($comment->isModified());
        $this->assertFalse($article->isModified());
        $info = $this->mandango->createDocument('Model\Info');
        $infos->add($info);
    ...
    public function testIsModifiedNotNewEmbeddedsMany()
    {
        ...
        $this->assertFalse($article->isModified());
        // add
        $addComment = $this->mandango->createDocument('Model\Comment');
    ...
    public function testClearModifiedFields()
    {
        ...
        $article = $this->mandango->createDocument('Model\Article')->setDocumentData(array('_id' => new \MongoId('1'), 'title' => 'foo'))->setTitle('bar');
        $article->clearModified();
    ...
    {
        $source = $this->mandango->createDocument('Model\Source')->setName('foo');
        $article = $this->mandango->createDocument('Model\Article')->setSource($source);
        $article->clearModified();
        ...
        $article = $this->mandango->createDocument('Model\Article')->setDocumentData(array('_id' => new \MongoId('1'), 'source' => array('name' => 'foo')));
        $source = $article->getSource();
        $source->setName('bar');
        $article->setSource(null);
        $article->clearModified();
    ...
    public function testClearModifiedEmbeddedsMany()
    {
        $article = $this->mandango->createDocument('Model\Article');
        $comments = $article->getComments();
        $comment = $this->mandango->createDocument('Model\Comment')->setName('foo');
        $article->clearModified();
        ...
        $comments = $article->getComments();
        $savedComments = $comments->getSaved();
        $comments->add($this->mandango->createDocument('Model\Comment')->setName('foobar'));
        $article->clearModified();
        $this->assertSame(array(), $comments->getAdd());
        $this->assertFalse($comments->isSavedInitialized());
        $this->assertFalse($article->isModified());
    ...
    {
        $article = $this->mandango->createDocument('Model\Article')->setTitle('foo')->setNote(null);
        $this->assertTrue($article->isFieldModified('title'));
        $this->assertFalse($article->isFieldModified('content'));
        $this->assertFalse($article->isFieldModified('note'));

        $source = $this->mandango->createDocument('Model\Source')->setName('foo')->setNote(null);
    ...
    {
        $source = $this->mandango->createDocument('Model\Source')->setInfo($this->mandango->createDocument('Model\Info'));
    ...
    public function testGetOriginalEmbeddedOneValueWithoutValue()
    {
        ...
        $source = $this->mandango->createDocument('Model\Article');
        $this->assertNull($source->getOriginalEmbeddedOneValue('info'));
    }

    public function testGetOriginalEmbeddedOneValueNew()
    {
        $article = $this->mandango->createDocument('Model\Article')->setSource($this->mandango->createDocument('Model\Source'));
        $this->assertNull($article->getOriginalEmbeddedOneValue('source'));
        $source = $this->mandango->createDocument('Model\Source')->setInfo($this->mandango->createDocument('Model\Info'));
        $this->assertNull($source->getOriginalEmbeddedOneValue('info'));
    ...
    public function testGetEmbeddedsOneNotNew()
    {
        ...
        $source = $this->mandango->createDocument('Model\Source')->setDocumentData(array(
            'info' => array('name' => 'bar'),
        ));
        $this->assertSame(array(), $source->getEmbeddedsOneChanged());
